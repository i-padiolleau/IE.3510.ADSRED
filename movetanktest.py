...
motor_xaxis = LargeMotor(OUTPUT_A)
...
def turn_robot(angle):
    motor_xaxis.on_for_degrees(speed=MOTOR_SPEED, degrees=angle)
    sleep(1)
    motor_xaxis.wait_until_not_moving()  # Wait until the motor stops

# Function to turn the robot up and down by a specified angle for both motors
def turn_up_down(angle):

    tank_pair.on_for_degrees(left_speed=MOTOR_SPEED, right_speed=MOTOR_SPEED, degrees=angle)
    sleep(1)
...
while True:
    ...
    if detected_objects:
        ...
        sleep (1)
        turn_robot(-TURN_ANGLE)  # Turn the robot 180 degrees back to position
        sleep (1)

        print("Turning up and down 90 degrees.")
        turn_up_down(TURN_ANGLE_UP_DOWN)  # Turn the robot arm up 90 degrees
        sleep (1)
        turn_up_down(-TURN_ANGLE_UP_DOWN)  # Turn the robot arm down by 90 degrees
        sleep (1)